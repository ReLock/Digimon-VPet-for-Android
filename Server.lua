...
local function connectClients() --checks to see if any new clients wish to connect
	...
	message,clientIP,clientPort = UDPBroadcaster:receivefrom() --from handshake client
...
local function mainLoop() --handles both data transfer, ACKS, disconnecting clients
	...
	--send all messages, unified buffer--------------------------------------
	--		Runtime:dispatchEvent({name = "autolanUpdate"})
	if(sendCredits>0) then
	--[[
		while(bufferIndexLowSend~=bufferIndexLow) do
			local clientIndex = sendQueueLowClients[bufferIndexLowSend] --ie player number
	...
	-------------------------files	
		...
		--[[
	---------------------------------- HIGH PRIORITY
		while(bufferIndexHighSend~=bufferIndexHigh) do
			local clientIndex = sendQueueHighClients[bufferIndexHighSend] --ie player number
			local clientInfo = clientsInfo[clientIndex]
			if(clientInfo ~= nil) then
				local UDPClient = UDPClients[clientIndex]	
				numMessagesRecieved = 0
				...
				packetTemplate[2] ={2,clientInfo.numMessages,bufferIndexHighSend} -- control/acks
				packetTemplate[3] = clientInfo.HighPriorityRecieved --high priority acks	
				clientInfo.numHighPriorityRecieved = 0
				clientInfo.HighPriorityRecieved = {0}	
				
				HighPriorityCounters[bufferIndexHighSend] = HighPriorityCount --controls when to resend
				sendQueueHigh[bufferIndexHighSend] = packetTemplate --to prevent reencoding upon resend
