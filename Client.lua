...
local function mainLoop()
	--must have a valid connected peer
	--first receive----
	--recieve()
	--Runtime:dispatchEvent({name = "autolanUpdate"})	
------------------------SEND			
	if(sendCredits >0) then
		--check circular buffer and start sending
		--numMessagesRecieve1 = numMessagesRecieved --buffer so that we send meaningful acks and redundant if packet loss
	--[[	while(bufferIndexLowSend~=bufferIndexLow and sendCredits>0) do
			if(UDPClient) then
				heartbeatTimer = heartbeatTime 
				local packetTemplate = {}
				packetTemplate[1] = sendQueueLow[bufferIndexLowSend] --payload
				packetTemplate[2] ={1,numMessagesRecieved} --flow control/acks
				packetTemplate[3] = HighPriorityRecieved --high priority acks	
				numHighPriorityRecieved = 0 			
				HighPriorityRecieved = {0}					
		...
		--[[		
		while(bufferIndexHighSend~=bufferIndexHigh and sendCredits>0) do
	--		numMessagesRecieved = 0
	--		heartbeatTimer = heartbeatTime 
			if(UDPClient) then
				local packetTemplate = {}
				packetTemplate[1] = sendQueueHigh[bufferIndexHighSend] --payload
				packetTemplate[2] ={2,numMessagesRecieved,bufferIndexHighSend} -- control/acks
				packetTemplate[3] = HighPriorityRecieved --high priority acks	
				numHighPriorityRecieved = 0 	
				numMessagesRecieved = 0		
				HighPriorityRecieved = {0}	
				
				HighPriorityCounters[bufferIndexHighSend] = HighPriorityCount --controls when to resend
				sendQueueHigh[bufferIndexHighSend] = packetTemplate
				if(sendQueueHighCallbacks[bufferIndexHighSend]) then
					sendQueueHighCallbacks[bufferIndexHighSend]({phase = "began"})
				end
				UDPClient:send( json.encode(packetTemplate) ) --send data entry
				sendCredits = sendCredits-1
			end
			bufferIndexHighSend = bufferIndexHighSend+1
			if(bufferIndexHighSend == circularBufferSize) then
				bufferIndexHighSend=1
			end		
		end	--]]
...
local function connectToServer()
	...
	--	UDPClient:setpeername(serverIP, availibleServers[serverIP].broadcastPort) --we send to broadcast port b/c we do not want to do recieveFrom() function in main loop
	--	sendPhase = true 
